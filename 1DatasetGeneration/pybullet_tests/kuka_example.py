    ...
    def simulate(self):
        for t in range(4000):
            ...
            if t % 8 == 0:  # PyBullet default simulation time step is 240fps.
                cam_view_matrix = p.computeViewMatrixFromYawPitchRoll(C.cam_target_pos, C.cam_distance, C.cam_yaw, C.cam_pitch,
                                                                      C.cam_roll, C.cam_up_axis_idx)
                cam_projection_matrix = p.computeProjectionMatrixFOV(C.cam_fov, C.cam_width * 1. / C.cam_height, C.cam_near_plane,
                                                                     C.cam_far_plane)
