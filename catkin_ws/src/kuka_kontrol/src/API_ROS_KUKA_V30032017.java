...
public class API_ROS_KUKA_V30032017 extends RoboticsAPIApplication {
	...
	public void run() {
		...
		while( RUN )
		{
			...
			else if( (lineSplt[0].toString()).equals("setPositionXYZABC".toString()))
				setPositionXYZABC(CommandStr);
			else if( (lineSplt[0].toString()).equals("MoveXYZABC".toString()))
				MoveXYZABC(CommandStr);
			else if( (lineSplt[0].toString()).equals("setCompliance".toString()))
				setCompliance(CommandStr);
			else if( (lineSplt[0].toString()).equals("resetCartImpCtrl".toString()))
				resetCartImpCtrl();
			else if( (lineSplt[0].toString()).equals("resetCompliance".toString()))
				resetCompliance();
			else if( (lineSplt[0].toString()).equals("setCartImpCtrl".toString()))
				setCartImpCtrl(CommandStr);
			else if( (lineSplt[0].toString()).equals("setTool".toString()))
				setTool(CommandStr);
			else if( (lineSplt[0].toString()).equals("setJointAcceleration".toString()))
				setJointAcceleration(CommandStr);
			else if( (lineSplt[0].toString()).equals("setJointVelocity".toString()))
				setJointVelocity(CommandStr);
			else if( (lineSplt[0].toString()).equals("setJointJerk".toString()))
				setJointJerk(CommandStr);
			else if( (lineSplt[0].toString()).equals("setCartVelocity".toString()))
				CartVelocity(CommandStr);
			else if( (lineSplt[0].toString()).equals("resetCollision".toString()))
				isCollision = false;
			else if( (lineSplt[0].toString()).equals("forceStop".toString()))
				setForceStop();
			else if( (lineSplt[0].toString()).equals("MoveCirc".toString()))
                MoveCirc(CommandStr);
			else if( (lineSplt[0].toString()).equals("sleep".toString()))
				sleep(CommandStr);
