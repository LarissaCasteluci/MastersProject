...
for i in range(1000):
    ...
    print('OperatorAck\t=', kuka_ros_node.OperatorAck)            # Reading OperatorAck

    kuka_ros_node.send_command('askForOperatorAck Allow Movement')
    time.sleep(.1)

    kuka_ros_node.send_command('setTool tool1')
    
    while kuka_ros_node.OperatorAck[0] == False:
        time.sleep(.1)
