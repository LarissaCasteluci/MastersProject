...
for i in range(1000):
    ...
    print('OperatorAck\t=', my_client.OperatorAck)            # Reading OperatorAck

    my_client.send_command('askForOperatorAck Allow Movement') 
    time.sleep(.1)

    my_client.send_command('setTool tool1')
    
    print('antes')
    while my_client.OperatorAck[0] == False:
        time.sleep(.1)
        print('meio: ', my_client.OperatorAck[0])
